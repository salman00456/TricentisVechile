...
public class SendQuote extends Base{
	...
	@Test
	public static void sendQuoteDetails()throws InterruptedException {
		...
		// Enter start date
		WebElement startDate = driver.findElement(By.xpath(prop.getProperty("startDateTxt")));
		Assert.assertTrue(startDate.isDisplayed());
		startDate.sendKeys("12/05/2023");

		// Select insurance sum
		Thread.sleep(3000);
		WebElement insuranceSum = driver.findElement(By.xpath(prop.getProperty("selectInsuranceSum")));
		Assert.assertTrue(insuranceSum.isEnabled());
		Select selectSum = new Select(insuranceSum);
		selectSum.selectByValue("3000000");

		// Select merit rating
		WebElement meritRating = driver.findElement(By.xpath(prop.getProperty("selectMerit")));
		Assert.assertTrue(meritRating.isEnabled());
		Select selectMerit = new Select(meritRating);
		selectMerit.selectByValue("Bonus 1");

		// select damage insurance
		WebElement damageInsurance = driver
				.findElement(By.xpath(prop.getProperty("selectDamageIns")));

		Assert.assertTrue(damageInsurance.isEnabled());

		Select selectDamage = new Select(damageInsurance);
		selectDamage.selectByValue("Partial Coverage");

		// enter optional products
		Actions act1 = new Actions(driver);

		act1.moveToElement(driver.findElement(By.xpath(prop.getProperty("optionalPrdctTxt")))).click().perform();

		// select courtesy car
		WebElement courtesyCar = driver.findElement(By.xpath(prop.getProperty("selectCourtsey")));
		Assert.assertTrue(courtesyCar.isEnabled());

		Select selectCar = new Select(courtesyCar);
		selectCar.selectByVisibleText("Yes");

		// click on next button
		WebElement nextButton1 = driver.findElement(By.xpath(prop.getProperty("priceNxtBtn")));
		Assert.assertTrue(nextButton1.isEnabled());
		nextButton1.click();

		// Select gold plan
		Actions selectGoldPlan =  new Actions(driver);

		selectGoldPlan.moveToElement(driver.findElement(By.xpath(prop.getProperty("selectGoldPlan")))).click().perform();

		//click on the send quote button

		WebElement sendQuoteButton1;
		WebDriverWait wait1 = new WebDriverWait(driver,3000);
		sendQuoteButton1 = wait1.until
				(ExpectedConditions.visibilityOfElementLocated(By.xpath(prop.getProperty("clickSendQuoteBtn"))));

		Assert.assertTrue(sendQuoteButton1.isEnabled());
		sendQuoteButton1.click();

		//Enter e-mail
		WebElement email = driver.findElement(By.xpath(prop.getProperty("emailTxt")));
		Assert.assertTrue(email.isDisplayed());
		email.sendKeys("xyz@gmail.com");

		//Enter phone number
		WebElement phoneNumber = driver.findElement(By.xpath(prop.getProperty("phoneTxt")));
		Assert.assertTrue(phoneNumber.isDisplayed());
		phoneNumber.sendKeys("8876764345");

		//Enter Ussername
		WebElement username = driver.findElement(By.xpath(prop.getProperty("ussernameTxt")));
		Assert.assertTrue(username.isDisplayed());
		username.sendKeys("tvs.78");

		//Enter Password
		WebElement password = driver.findElement(By.xpath(prop.getProperty("passwordTxt")));
		Assert.assertTrue(password.isDisplayed());
		password.sendKeys("tricentis1234");

		//Enter confirm password
		WebElement confirmPassword = driver.findElement(By.xpath(prop.getProperty("confrmPassTxt")));
		Assert.assertTrue(confirmPassword.isDisplayed());
		confirmPassword.sendKeys("tricentis1234");

		//Enter comments
		WebElement coments = driver.findElement(By.xpath(prop.getProperty("commentTxt")));
		Assert.assertTrue(coments.isDisplayed());
		coments.sendKeys("its a very good website");
